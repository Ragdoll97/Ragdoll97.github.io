...
<li class="breadcrumb-item">
    <a href="{{ url('/admin/users/all') }}" ><i class="fas fa-user-cog"></i>Editar Usuario : {{$u->name}}</a>
</li>
@endsection
                            ...
                            <div class="mini_profile">
                               @if (is_null($u->avatar))
                                    <img src="{{ url('/static/images/avatar-icon2.png') }}"
                                        class="rounded-circle  img-fluid avatar" height="40">
                                @else
                                    <img src="{{ url('uploads_users/'.$u->id.'/av_'.$u->avatar) }}"
                                        class="rounded-circle  img-fluid avatar"  height="40">
                                @endif 
                                <div class="info ">
                                ...
                                @if ($u->status == '100')
                                    @if (KeyValueFromJson(Auth::user()->permissions, 'user_banned'))
                                        <a href="{{ url('/admin/users/' . $u->id . '/banned') }}"
                                            class="btn btn-success">Activar Usuario</a>
                                    @endif
                                @else
                                    @if (KeyValueFromJson(Auth::user()->permissions, 'user_banned'))
                                        <a href="{{ url('/admin/users/' . $u->id . '/banned') }}"
                ...
                <div class="col-md-8">
                    <div class="panel shadow">
                        ...
                        <div class="inside">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <label for="user_type" class="mtop16">Tipo de usuario:</label>
